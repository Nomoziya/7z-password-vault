...
原计划的功能实现与本轮验收已完成。`1.6.0-rc1` 未签名 portable 包已从独立干净检出生成，复用同哈希已测 EXE，重新生成当前文档、清单和构建来源记录；未上传 GitHub 或发布正式标签。

最终交付位于 `dist/v1.6.0-rc1-final-20260925/`，打包提交 `a4800dc35829d83bf186f689dc3f13aaec8aa2a6`，`sourceDirty=false`。新 ZIP 验收通过，15 项 SBOM 和 1494 项源码清单均已核对，原始证据在干净检出后字节不变。下载文件名、完整哈希和源码归档见 `release-1.6.0-rc1.md`，核验结果见 `restore-rc1-package-validation.json`。原始 dev4 包不变；本次没有因文档更新重跑相同程序的 GUI 或原生测试。
