...
已实现恢复 API、跨进程会话协调、按钮与确认窗口、三种语言资源、原生故障用例和 GUI 恢复用例。最新普通桌面完整 GUI 为 426/0；独立标准账户与最终候选证据仍在收集，详细状态见 `restore-validation.md`。没有重新发布正式版本。原计划中的启动残留材料提示、清理失败单独报告及部分专项验收仍未完成，不计作已交付。
