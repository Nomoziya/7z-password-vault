...
dev3 的普通桌面及独立标准账户完整 GUI 426/0、升级回滚、真实磁盘满与干净重建均通过。随后 dev4 补齐启动残留提示、清理失败单独报告及注册表故障 GUI 验收，三种语言在实际 150% 缩放的恢复专项各 45/45，完整普通桌面 GUI 436/0，干净重建与包验收通过。仅剩最终候选的独立标准账户/跨账户 DPAPI 和管理员磁盘满证据，已按用户要求延后至返回后。没有重新发布正式版本，历史通过结果不冒充新程序的实测。
