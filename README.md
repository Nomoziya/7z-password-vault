...
- 已验收范围：Windows 11 Insider x64（build 26220）。独立标准用户在相同哈希的两个 GUI 程序上完成 436/436 项交互及升级回滚检查，并验证跨账户 DPAPI 备份拒绝恢复。简体中文完整回归覆盖 150%/200% 缩放，繁体中文及英文恢复专项覆盖 150%；其他 Windows 版本与未测缩放不作保证。原始结果及限制见源码仓库 `docs/restore-final-validation.md`。
- 简体中文、繁体中文、英文。
...
启动时若发现当前库对应的恢复临时文件，会提示其路径供检查，不会自动使用或删除。清理临时密文失败也会单独报告路径与错误，不改变已经报告的恢复结果。正常的恢复前副本会一直保留，不会反复触发中断提示。
